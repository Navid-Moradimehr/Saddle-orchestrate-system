...
const nodes: FlowNode[] = [
  { id: 'people', label: 'Plant & customer context', eyebrow: 'INPUT', detail: 'Signals · constraints', x: 14, y: 30, color: '#5EA7FF' },
  { id: 'question', label: 'Decision point', eyebrow: 'ENTRY', detail: 'Outcome · risk', x: 14, y: 66, color: '#5EA7FF' },
  ...
  { id: 'system', label: 'Operational loop', eyebrow: 'AUTOMATE', detail: 'MCP · routing · approvals', x: 70, y: 44, color: '#34D399' },
  { id: 'launch', label: 'Customer outcome', eyebrow: 'LIVE', detail: 'Ship · observe', x: 86, y: 26, color: '#34D399' },
  { id: 'feedback', label: 'Feedback & learning', eyebrow: 'LOOP', detail: 'Gradients · signals', x: 86, y: 71, color: '#F47DBB' },
];
  ...
  return (
    <section ref={sectionRef} id="workflow" className={`relative overflow-hidden px-6 py-24 md:py-32 ${visualMode === 'human' ? 'bg-orange/[0.035]' : 'bg-grid-light dark:bg-grid-dark'}`} aria-labelledby="workflow-heading">
      <div className="mx-auto max-w-[1280px]">
        ...
        <div className={fullscreen ? 'fixed inset-0 z-[80] flex flex-col overflow-hidden bg-[#07101d] p-3 shadow-2xl sm:p-6' : 'overflow-x-auto rounded-[1.75rem] border border-border-dark/80 bg-[#07101d] p-3 shadow-[0_28px_80px_rgba(2,11,20,.22)] dark:border-white/10 md:p-5'} role={fullscreen ? 'dialog' : undefined} aria-modal={fullscreen ? true : undefined} aria-label={fullscreen ? 'Full-screen living process map' : undefined}>
          <div className={fullscreen ? 'relative min-h-0 min-w-0 flex-1 overflow-hidden rounded-[1.25rem] border border-white/10 bg-[linear-gradient(rgba(95,150,210,.07)_1px,transparent_1px),linear-gradient(90deg,rgba(95,150,210,.07)_1px,transparent_1px)] bg-[size:44px_44px]' : 'relative min-h-[520px] min-w-[760px] overflow-hidden rounded-[1.25rem] border border-white/10 bg-[linear-gradient(rgba(95,150,210,.07)_1px,transparent_1px),linear-gradient(90deg,rgba(95,150,210,.07)_1px,transparent_1px)] bg-[size:44px_44px]'} aria-label="Interactive workflow map" role="region">
            <div className="absolute left-4 top-4 z-30 flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.18em] text-white/50"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_10px_#34D399]" /> Live</div>
            <button type="button" onClick={() => setFullscreen((value) => !value)} aria-label={fullscreen ? 'Exit full-screen workflow' : 'View workflow full screen'} className="absolute right-4 top-3 z-30 inline-flex items-center gap-2 rounded-full border border-white/15 bg-[#07101d]/80 px-3 py-2 font-mono text-[9px] uppercase tracking-[0.14em] text-white/70 backdrop-blur transition-colors hover:border-orange hover:text-white focus-visible:outline-orange">
