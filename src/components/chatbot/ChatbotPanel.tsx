...
export default function ChatbotPanel({ open, state, onClose, onAsk, messages }: Props) {
  ...
  return <AnimatePresence>
    {open && <motion.div
      ...
      className="fixed bottom-[calc(max(1rem,env(safe-area-inset-bottom))+6rem)] right-[max(1rem,env(safe-area-inset-right))] z-50 flex max-h-[min(680px,calc(100dvh-7.5rem))] w-[min(380px,calc(100vw-2rem))] origin-bottom-right flex-col overflow-hidden overscroll-contain rounded-2xl border border-border bg-cream shadow-2xl dark:border-border-dark dark:bg-charcoal"
    >
