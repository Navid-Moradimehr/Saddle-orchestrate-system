...
function cardMotion(offset: number) {
  ...
  return {
    ...
    filter: offset === 0 ? 'none' : 'saturate(0.7) brightness(0.8)',
...
export default function ServicesSection() {
  ...
  const select = useCallback((index: number) => setActive((index + services.length) % services.length), []);
  const next = useCallback(() => setActive((index) => (index + 1) % services.length), []);
  const previous = useCallback(() => setActive((index) => (index - 1 + services.length) % services.length), []);
        ...
        <div ref={regionRef} tabIndex={0} role="region" aria-roledescription="carousel" aria-label="Services" className="focus-visible:outline-orange">
          <div className="relative mx-auto h-[510px] max-w-[1120px] overflow-visible [perspective:1400px] md:h-[560px]">
            <motion.div
              ...
              onDragEnd={(_, info) => { setDragging(false); if (Math.abs(info.offset.x) > 42 || Math.abs(info.velocity.x) > 280) (info.offset.x < 0 ? next : previous)(); }}
              ...
              {services.map((service, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div className={`relative flex h-full flex-col overflow-hidden rounded-[1.55rem] border border-white/20 bg-gradient-to-b from-white/90 via-white/70 to-cream/80 p-5 dark:border-white/10 dark:from-charcoal-light/95 dark:via-charcoal-light/80 dark:to-charcoal/95 md:p-7 ${!isActive ? 'max-sm:[&>*:not(:first-child)]:opacity-0' : ''}`}>
